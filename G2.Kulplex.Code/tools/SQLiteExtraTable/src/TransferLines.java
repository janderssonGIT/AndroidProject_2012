...
public class TransferLines {
	...
	public static void updateL(int[][] a, int[][] b) throws SQLException{
		...
		for(int i = 0; i< ReceiveLines.rowCount; i++){
			...
			for(int j = 0; j < a[i].length; j++){
				...
				s2.executeUpdate("INSERT INTO planet_osm_streets VALUES (" + (k+1) + ", " + (i+1) + ", '" + a[i][j] + "', '" + b[i][j] + "')");
				
				k++;
			}
			
			
		System.out.println((i+1) + "/" + ReceiveLines.rowCount);
		}
